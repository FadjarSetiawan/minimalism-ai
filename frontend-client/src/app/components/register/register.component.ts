      ...
      <div class="w-full max-w-sm">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input type="password" [(ngModel)]="password" (keydown.enter)="onRegister()" placeholder="Minimal 6 karakter"
                   class="w-full border border-border dark:border-zinc-800 rounded-2xl px-4 py-3 text-[14px] text-primary dark:text-[#ececf1] placeholder-secondary dark:placeholder-zinc-700 bg-white dark:bg-zinc-900 focus:outline-none focus:border-accent dark:focus:border-[#38bdf8] transition-all">
          </div>
        ...
        <p class="text-center text-[13px] text-secondary dark:text-zinc-400 mt-6 font-sans select-none">
          Sudah punya akun? 
          <a routerLink="/login" class="text-accent dark:text-[#38bdf8] font-medium no-underline hover:opacity-85">Masuk</a>
        </p>
      </div>
    </div>
  `
...
export class RegisterComponent {
  ...
  async onRegister() {
    ...
    if (this.password.length < 6) {
      this.errorMsg = 'Kata sandi harus minimal 6 karakter.';
      return;
    }
    this.isLoading = true;
    ...
    if (result.success) {
      ...
      this.errorMsg = result.error || 'Gagal mendaftar.';
    }
  }
}
